...
export const CardPage = () => {
  ...
  // Для поиска нужно добавлять функцию debounce что бы при каждом нажатие на кнопку не отправлялся запрос
  useEffect(() => {
    fetchData({
  ...
  return (
    <>
      ...
      <div className="container">
        {/* Делай всегда строгую проверку это убережет от нежелаемых ошибок */}
        {cards.docs == null ? (
          <Loading />
        ) : // Не делай такую вложеность тертарных операторов, лучше вверху сделать if(cards.docs) return <Loading /> так и код короче и нет такой вложености
        cards.docs.length === 0 ? (
